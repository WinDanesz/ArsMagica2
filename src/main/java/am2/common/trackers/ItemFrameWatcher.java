...
public class ItemFrameWatcher {
                    ...
                    if (CompendiumDiscoveryHandler.isPartOf2x2Pool(frame.world, frame.getPosition().add(i, j, k))) {
                        ...
                        if (time >= processTime) {
                            if (!frame.world.isRemote) {
                                frame.setDisplayedItem(new ItemStack(AMItems.arcane_compendium));
                                shouldRemove = true;
                                break;
                            }
                        } else {
    ...
    private void updateQueuedChanges() {
        ...
        for (EntityItemFrameComparator comp : toAdd) {
            if (comp.frame != null && (comp.frame.getDisplayedItem() == null || comp.frame.getDisplayedItem().getItem() != AMItems.arcane_compendium))
        ...
        for (EntityItemFrameComparator comp : toRemove) {
            ...
            if (time != null && time >= processTime &&
                    ...
                    (comp.frame.getDisplayedItem() != null &&
                            (comp.frame.getDisplayedItem().getItem() == Items.BOOK || comp.frame.getDisplayedItem().getItem() == AMItems.arcane_compendium))) {
    ...
    public void spawnCompendiumProgressParticles(EntityItemFrame frame, BlockPos pos) {
        ...
        AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(frame.world, "symbols", pos.getX() + rand.nextDouble(), pos.getY() + rand.nextDouble(), pos.getZ() + rand.nextDouble());
        if (particle != null) {
    ...
    public void spawnCompendiumCompleteParticles(EntityItemFrame frame) {
        AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(frame.world, "radiant", frame.posX, frame.posY, frame.posZ);
        if (particle != null) {
