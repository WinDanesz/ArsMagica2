        ...
        if (player.isWet() && !player.world.isRemote) {
            if (player.getRNG().nextFloat() < 0.04f) {
                ...
                if (player.world.isRemote) {
                    ArsMagica.proxy.particleManager.BoltFromEntityToPoint(player.world, player, player.posX - 2 + player.getRNG().nextDouble() * 4, player.posY + player.getEyeHeight() - 2 + player.getRNG().nextDouble() * 4, player.posZ - 2 + player.getRNG().nextDouble() * 4);
                }
            }
//			else{
