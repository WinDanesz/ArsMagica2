...
/**
 * Spell component that applies the {@code ancientspellcraft:shrinkage} potion effect to a target.
 *
 ...
 */
public class Shrinkage extends SpellComponent {
    ...
    @Nullable
    private static Potion shrinkagePotion;

    @Nullable
    private static Potion getPotion() {
        if (shrinkagePotion == null) {
            shrinkagePotion = ForgeRegistries.POTIONS.getValue(new ResourceLocation("ancientspellcraft", "shrinkage"));
        }
        return shrinkagePotion;
    }
    ...
    public Object[] getRecipe() {
        return new Object[]{ new ItemStack(Items.FERMENTED_SPIDER_EYE) };
    }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (particle != null) {
                ...
                particle.addVelocity(
                        ...
                        (rand.nextDouble() - 0.5) * 0.08);
                particle.setParticleAge(8 + rand.nextInt(5));
                // Purple–blue hue: arcane shrinkage
                particle.setRGBColorF(0.4f + rand.nextFloat() * 0.3f, 0.0f, 0.9f + rand.nextFloat() * 0.1f);
                particle.AddParticleController(new ParticleFloatUpward(particle, 0.05f, 0.01f, 1, false));
            }
        }
    }
