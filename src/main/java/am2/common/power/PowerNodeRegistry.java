...
public class PowerNodeRegistry {
    ...
    static int POWER_SEARCH_RADIUS = 10; //The literal power search radius
    static int POWER_SEARCH_RADIUS_SQ = 100; //The power search radius squared.  This is used for faster vector math.
    static int MAX_POWER_SEARCH_RADIUS = 10000; //The maximum power search radius (squared)

    public static void applyConfig() {
        POWER_SEARCH_RADIUS = ArsMagica.config.getPowerSearchRadius();
        POWER_SEARCH_RADIUS_SQ = POWER_SEARCH_RADIUS * POWER_SEARCH_RADIUS;
        int maxLinear = ArsMagica.config.getMaxPowerSearchRadius();
        MAX_POWER_SEARCH_RADIUS = maxLinear * maxLinear;
    }
    ...
    PowerNodeEntry registerPowerNodeInternal(IPowerNode<?> node) {
        ...
        //prevent duplicate registrations (entry may already exist from saved chunk data)
    ...
    public IPowerNode<?>[] getAllNearbyNodes(World world, BlockPos location, PowerTypes power) {
        ...
        for (BlockPos vector : nodesToSearch.keySet()) {
            if (world.getChunk(vector) == null) {
                continue;
            }
            Chunk chunk = world.getChunk(vector);
            if (!chunk.isLoaded())
                continue;
...
//		if (te.getWorld() == null)
//			return null;
//		if (te.getWorld().getChunk(te.getPos()) == null)
        return new ChunkPos(te.getPos().getX() >> 4, te.getPos().getZ() >> 4);
//		return te.getWorld().getChunk(te.getPos()).getChunkCoordIntPair();
    }
...
//	}
    ...
    public static void unloadDimension(World world) {
        HashMap<Integer, PowerNodeRegistry> dimensionPowerManagers = world.isRemote ? clientDimensionPowerManagers : serverDimensionPowerManagers;
        dimensionPowerManagers.remove(world.provider.getDimension());
    }
}
