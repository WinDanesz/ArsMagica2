...
public class WitchwoodTreeHuge extends WorldGenBigTree {
        ...
        for (aint1[par5] = aint[par5]; j1 <= i1; ++j1) {
            ...
            while (k1 <= i1) {
                ...
                if (d0 > (double) (par4 * par4)) {
                    ...
                    aint1[b2] = aint[b2] + k1;
                    BlockPos blockPos = new BlockPos(aint1[0], aint1[1], aint1[2]);
                    if (!this.worldObj.isBlockLoaded(blockPos)) {
                        ++k1;
                        continue;
                    }
                    Block l1 = this.worldObj.getBlockState(blockPos).getBlock();

                    if (l1 != Blocks.AIR && l1 != AMBlocks.witchwood_leaves) {
                        ++k1;
                    } else {
                        worldObj.setBlockState(blockPos, par6, 2);
                        ++k1;
                    }
                }
            }
    ...
    void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, IBlockState par3) {
        ...
        if (aint2[b1] != 0) {
            ...
            for (int k = aint2[b1] + b4; j != k; j += b4) {
                ...
                BlockPos blockPos = new BlockPos(aint3[0], aint3[1], aint3[2]);
                if (!worldObj.isBlockLoaded(blockPos)) continue;
    ...
    void generateTrunk() {
        ...
        this.placeBlockLine(aint, aint1, AMBlocks.witchwood_log.getDefaultState());
    ...
    void generateLeafNodeBases() {
        ...
        for (int[] aint = new int[]{this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i) {
            ...
            if (this.leafNodeNeedsBase(k)) {
                this.placeBlockLine(aint, aint2, AMBlocks.witchwood_log.getDefaultState());
            }
        }
    }
    ...
    int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger) {
        ...
        if (aint2[b1] == 0) {
            ...
            for (j = aint2[b1] + b4; i != j; i += b4) {
                ...
                BlockPos blockPos = new BlockPos(aint3[0], aint3[1], aint3[2]);
                if (!this.worldObj.isBlockLoaded(blockPos)) continue;
                Block k = this.worldObj.getBlockState(blockPos).getBlock();

                if (k != Blocks.AIR && k != AMBlocks.witchwood_leaves) {
                    break;
                }
            }
