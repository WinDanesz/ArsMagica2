    ...
    public void onUpdate() {
        ...
        if (world.isRemote) {
            if (getIsMoonstoneMeteor()) {
                AMParticle fire = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "explosion_2", posX, posY, posZ);
                if (fire != null) {
                    fire.setMaxAge(20);
                ...
                for (float i = 0; i < Math.abs(motionY); i += 0.1f) {
                    AMParticle star = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "ember", posX + motionX * i, posY + motionY * i, posZ + motionZ * i);
                    if (star != null) {
                        star.setMaxAge(22);
        ...
        if (getIsShootingStar()) {
            ...
            if (movingobjectposition.entityHit != null) {
                ...
                if (this.getIsMoonstoneMeteor()) {
                    ...
                    this.world.newExplosion(this, this.target.x, this.target.y, this.target.z, 0.8f, false, ArsMagica.config.moonstoneMeteorsDestroyTerrain());
...
//							if (((EntityPlayer)player).getDistanceSq(this) < 4096)
...
//					}
    ...
    private void generateSurfaceOreAtOffset(World world, BlockPos pos, boolean force) {
        ...
        if (rand.nextInt(4) < 2 || force)
            world.setBlockState(pos, AMBlocks.moonstone_ore.getDefaultState());
        else
