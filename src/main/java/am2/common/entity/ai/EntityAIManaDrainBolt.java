...
public class EntityAIManaDrainBolt extends EntityAIBase {
    ...
    private void doRangedAttack() {
        //43% chance to "miss"
        int chanceToMiss = entityHost.isPotionActive(MobEffects.SPEED) ? 10 : 43;
        if (world.rand.nextInt(100) < chanceToMiss) {
            ArsMagica.proxy.particleManager.BoltFromPointToPoint(world,
            ...
            ArsMagica.proxy.particleManager.BoltFromEntityToEntity(world, entityHost, entityHost, attackTarget, this.damage, 2, -1);
            ...
            if (manaDrained > 100) {
                ...
                if (entityHost.world.getDifficulty() == EnumDifficulty.HARD) {
                    attackTarget.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 40, 1, true, true));
                    entityHost.addPotionEffect(new PotionEffect(MobEffects.SPEED, 40, 3, true, true));
                }
            }
        }
    }
