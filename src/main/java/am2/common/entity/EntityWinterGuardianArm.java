...
public class EntityWinterGuardianArm extends EntityLiving {
    ...
    public EntityWinterGuardianArm(World par1World) {
        super(par1World);
        ticksExisted = 0;
        halflife = ArsMagica.config.getWinterArmFlightTicks();
        maxTicksToExist = halflife + 40;
        this.noClip = true;
        entityHit = null;
    }
        ...
        if (getThrowingEntity() != null) {
            if (getThrowingEntity() instanceof EntityWinterGuardian) {
                ...
                if (getThrowingEntity().getHealth() <= 0) {
                    PlayerTracker.storeSoulboundItemForRespawn((EntityPlayer) getThrowingEntity(), AMEnchantmentHelper.soulbindStack(new ItemStack(AMItems.winter_arm)));
                } else {
                    if (!((EntityPlayer) getThrowingEntity()).inventory.addItemStackToInventory(AMEnchantmentHelper.soulbindStack(new ItemStack(AMItems.winter_arm)))) {
                        EntityItem item = new EntityItem(world);
                        item.setPosition(posX, posY, posZ);
                        item.setItem(AMEnchantmentHelper.soulbindStack(new ItemStack(AMItems.winter_arm)));
    ...
    public void onUpdate() {
        ...
        if (isInWater()) {
            ...
            for (int i = 0; i < 2; ++i) {
                AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "ember", posX + rand.nextFloat() * 0.2 - 0.1, posY + 1.2, posZ + rand.nextFloat() * 0.2 - 0.1);
                if (particle != null) {
        ...
        if (movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityLivingBase) {
            ...
            if (getThrowingEntity() != null && this.entityHit == null) {
                ...
                if (movingobjectposition.entityHit instanceof EntityLivingBase) {
                    ...
                    ((EntityLivingBase) movingobjectposition.entityHit).addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, 60, 2));
                    ((EntityLivingBase) movingobjectposition.entityHit).addPotionEffect(new PotionEffect(MobEffects.MINING_FATIGUE, 60, 2));
                    ((EntityLivingBase) movingobjectposition.entityHit).addPotionEffect(new PotionEffect(AMPotions.frost_slow, 60, 2));
                }
            }
        }
    }
