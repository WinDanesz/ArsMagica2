    ...
    public void onUpdate() {
        ...
        if (getTicksToExist() - this.ticksExisted <= 5 && !hasGoneBoom) {
            ...
            if (!world.isRemote) {
                ...
                // Fix memory leak: avoid unnecessary array conversion
                ...
                for (int i = 0; i < 360; i += ArsMagica.config.FullGFX() ? 5 : ArsMagica.config.LowGFX() ? 10 : 20) {
                    AMParticle effect = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "ember", this.posX, this.posY, this.posZ);
                    if (effect != null) {
                        ...
                        effect.AddParticleController(
                                new ParticleLeaveParticleTrail(effect, "ember", false, 5, 1, false)
                                        ...
                                        .addRandomOffset(0.05f, 0.05f, 0.05f)
                        );
                    }
                }
            }
        ...
        if (getTicksToExist() - this.ticksExisted > 30) {
            ...
            for (EntityLivingBase e : players) {
                ...
                if (world.isRemote) {
                    ...
                    if (ArsMagica.config.LowGFX() && (this.ticksExisted % 4) != 0) {
                        break;
                    }
                    AMParticle effect = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "ember", e.posX, e.posY + (e.getEyeHeight() / 2), e.posZ);
                    if (effect != null) {
                        effect.setRGBColorF(0.24f, 0.24f, 0.8f);
