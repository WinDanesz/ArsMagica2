...
public class EntityHellCow extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(ArsMagica.config.getHellCowMaxHealth());
        this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.5D);
        this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(ArsMagica.config.getHellCowAttackDamage());
        this.getEntityAttribute(SharedMonsterAttributes.FOLLOW_RANGE).setBaseValue(50D);
    }

    @Override
    public int getTotalArmorValue() {
        return (int) ArsMagica.config.getHellCowArmor();
    }

    @Override
    protected ResourceLocation getLootTable() {
        return AMLoot.HELL_COW_LOOT;
    }
    ...
    protected void dropEquipment(boolean wasRecentlyHit, int lootingModifier) {
        ...
        if (i == 1 && wasRecentlyHit) {
            this.entityDropItem(((ItemHellCowHorn) AMItems.hell_cow_horn).createItemStack(), 0.0f);
        }
    }
