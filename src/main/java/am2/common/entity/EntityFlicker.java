...
public class EntityFlicker extends EntityAmbientCreature{
	...
	public EntityFlicker(World par1World){
		...
		// TODO
		//setFlickerType(ArsMagicaAPI.getAffinityRegistry().getValuesCollection()(getRNG()));
	}
...
//		TODO
//		this.dataManager.set(WATCHER_FLICKERTYPE, ArsMagicaAPI.getAffinityRegistry().getKey(affinity).toString());
	}

	public Affinity getFlickerAffinity(){
		return null;
//		TODO:
//		return ArsMagicaAPI.getAffinityRegistry().getValue(dataManager.get(WATCHER_FLICKERTYPE));
	}
	...
	public boolean getCanSpawnHere(){
		...
		if (Type.DEAD.equals(tagType) || Type.END.equals(tagType)) {
			validAffinities.add(Affinity.ENDER);
		} else if (Type.PLAINS.equals(tagType) || Type.FOREST.equals(tagType) || Type.CONIFEROUS.equals(tagType) || Type.JUNGLE.equals(tagType)) {
			validAffinities.add(Affinity.NATURE);
		} else if (Type.COLD.equals(tagType) || Type.SNOWY.equals(tagType)) {
			validAffinities.add(Affinity.ICE);
		} else if (Type.MAGICAL.equals(tagType)) {
			validAffinities.add(Affinity.ARCANE);
		} else if (Type.DRY.equals(tagType) || Type.HOT.equals(tagType) || Type.SAVANNA.equals(tagType) || Type.NETHER.equals(tagType)) {
			validAffinities.add(Affinity.FIRE);
		} else if (Type.OCEAN.equals(tagType)) {
			validAffinities.add(Affinity.LIGHTNING);

			validAffinities.add(Affinity.WATER);
		} else if (Type.SWAMP.equals(tagType) || Type.WATER.equals(tagType) || Type.RIVER.equals(tagType) || Type.WET.equals(tagType) || Type.BEACH.equals(tagType)) {
			validAffinities.add(Affinity.WATER);
		}
