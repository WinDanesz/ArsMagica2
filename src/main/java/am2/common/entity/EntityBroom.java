...
public class EntityBroom extends EntityCreature {
    ...
    public void onUpdate() {
        if (world.isRemote) {
            updateRotations();
            if (isMoving()) {
                AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "smoke", posX, posY, posZ);
                if (particle != null) {
    ...
    public void addItemStackToInventory(ItemStack stack) {
        boolean success = InventoryUtilities.mergeIntoInventory(inventory, stack);
    }
    ...
    public EnumActionResult applyPlayerInteraction(EntityPlayer player, Vec3d vec, EnumHand hand) {
        if (player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() == AMItems.attuning_staff) {
            if (this.world.isRemote) {
                ...
                this.entityDropItem(new ItemStack(AMItems.magic_broom), 0);
                dropInventoryItems();
                this.setDead();
            }
