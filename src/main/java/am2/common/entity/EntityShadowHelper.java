...
public class EntityShadowHelper extends EntityLiving {
    ...
    private void spawnParticles() {
        if (world.isRemote) {
            for (int i = 0; i < 25 * ArsMagica.config.getGFXLevel() + 1; ++i) {
                AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "arcane", posX, posY, posZ);
                if (particle != null) {
    ...
    public void setSearchLocationAndItem(AMVector3 location, ItemStack stack) {
        if (this.world.isRemote) return;
        this.dataManager.set(DW_SEARCH_ITEM, stack);
    ...
    public ResourceLocation getLocationSkin() {
        if (this.skinHelper == null) return ShadowSkinHelper.locationStevePng;
        ResourceLocation skin = this.skinHelper.getLocationSkin();
        return skin != null ? skin : ShadowSkinHelper.locationStevePng;
    }
