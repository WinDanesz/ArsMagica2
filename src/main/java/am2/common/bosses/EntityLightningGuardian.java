...
public class EntityLightningGuardian extends AM2Boss implements IAnimatedEntity {
    ...
    protected void initSpecificAI() {
        ...
        this.tasks.addTask(3, new EntityAICastSpell<EntityLightningGuardian>(this, NPCSpells.getInstance().lightningRune, 22, 27, 200, BossActions.CASTING));
        this.tasks.addTask(3, new EntityAICastSpell<EntityLightningGuardian>(this, NPCSpells.getInstance().scrambleSynapses, 45, 60, 300, BossActions.SMASH));
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(ArsMagica.config.getLightningGuardianMaxHealth());
    }
    ...
    public int getTotalArmorValue() {
        return (int) ArsMagica.config.getLightningGuardianArmor();
    }
            ...
            if (this.getCurrentAction() == BossActions.CHARGE) {
                if (ticksInCurrentAction > 50) {
                    for (int i = 0; i < 2 * ArsMagica.config.getGFXLevel(); ++i) {
                        AMParticle smoke = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "smoke", posX, posY + 4, posZ);
                        if (smoke != null) {
    ...
    protected ResourceLocation getLootTable() {
        return AMLoot.LIGHTNING_GUARDIAN_LOOT;
    }
