...
public class EntityAIOtherworldlyRoar extends AIAnimation {
    ...
    public boolean shouldAnimate() {
        ...
        if (living.world.getEntitiesWithinAABB(EntityLivingBase.class, living.getEntityBoundingBox().grow(9, 3, 9))
                .size() < 2) {
            return false;
        }
    ...
    public void updateTask() {
        ...
        if (guardian.getAttackTarget() != null) {
            if (guardian.getTicksInCurrentAction() == 33) {
                ...
                ISpellCaster spell = SpellCaster.of(NPCSpells.getInstance().enderGuardian_otherworldlyRoar);
                if (spell != null) {
                    spell.cast(NPCSpells.getInstance().enderGuardian_otherworldlyRoar, guardian.world, guardian);
                }
            } else {
                guardian.faceEntity(guardian.getAttackTarget(), 180, 180);
            }
