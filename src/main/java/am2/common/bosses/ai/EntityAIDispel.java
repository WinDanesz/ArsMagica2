...
public class EntityAIDispel extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        cooldownTicks--;
        boolean execute = ((IArsMagicaBoss) host).getCurrentAction() == BossActions.IDLE && !host.getActivePotionEffects().isEmpty() && cooldownTicks <= 0;
    ...
    public void updateTask() {
        ...
        if (boltTicks == 16) {
            ...
            ISpellCaster spell = SpellCaster.of(NPCSpells.getInstance().dispel);
            if (spell != null) {
                spell.cast(NPCSpells.getInstance().dispel, host.world, host);
            }
        }
        if (boltTicks >= 23) {
            resetTask();
