...
public class EntityAirGuardian extends AM2Boss {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(ArsMagica.config.getAirGuardianMaxHealth());
    }

    @Override
    public int getTotalArmorValue() {
        return (int) ArsMagica.config.getAirGuardianArmor();
    }
    ...
    public void onUpdate() {
            ...
            case SPINNING:
                ...
                if (this.world.isRemote) {
                    for (int i = 0; i < ArsMagica.config.getGFXLevel(); ++i) {
                        AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "wind", posX + world.rand.nextDouble() * 4 - 2, posY, posZ + world.rand.nextDouble() * 4 - 2);
                        if (particle != null) {
        ...
        if (this.posY < 150 && world.isRemote) {
            for (int i = 0; i < 25; ++i) {
                AMParticle wind = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "wind", posX, posY, posZ);
                if (wind != null) {
    ...
    protected ResourceLocation getLootTable() {
        return AMLoot.AIR_GUARDIAN_LOOT;
    }
