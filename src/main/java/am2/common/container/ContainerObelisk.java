...
public class ContainerObelisk extends Container {
    ...
    @Override
    @Nonnull
    public ItemStack transferStackInSlot(@Nonnull EntityPlayer par1EntityPlayer, int i) {
        ItemStack itemstack = ItemStack.EMPTY;
        Slot slot = inventorySlots.get(i);
        if (slot != null && slot.getHasStack()) {
            ItemStack itemstack1 = slot.getStack();
            itemstack = itemstack1.copy();
            int PLAYER_ACTION_BAR_END = 37;
            int PLAYER_ACTION_BAR_START = 28;
            int PLAYER_INVENTORY_START = 1;
            int INVENTORY_STORAGE_START = 0;
