...
import am2.network.packets.PacketSetKeystoneCombo;
...
public class ContainerKeystone extends Container {
    ...
    public ContainerKeystone(InventoryPlayer inventoryplayer, ItemStack bookStack, ItemStack runeBagStack, InventoryKeyStone inventoryKeystone, InventoryRuneBag runeBag, int runeBagSlot) {
        ...
        if (this.runeBag != InventoryRuneBag.EMPTY) {
            int runeSlotIndex = 0;
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 2; j++) {
    ...
    public KeystoneCombination getCurrentMatchedCombination() {
        int savedCombos = ItemKeystone.numCombinations(keystoneStack);
        ...
        for (int c = 0; c < InventoryKeyStone.inventorySize; ++c) {
            ...
            curMeta[c] = stack != ItemStack.EMPTY ? stack.getItemDamage() : -1;
        }

        for (int i = 0; i < savedCombos; ++i) {
            KeystoneCombination currentCombo = ((ItemKeystone) AMItems.keystone).getCombinationAt(keystoneStack, i);
    ...
    public boolean setInventoryToCombination(int comboIndex) {
        ...
        KeystoneCombination combo = ((ItemKeystone) AMItems.keystone).getCombinationAt(keystoneStack, comboIndex);
        if (combo == null) return false;

        if (!inventoryContainsAllMetas(combo.metas)) return false;

        if (player.world.isRemote) {
            AMNetworkHandler.getNetwork().sendToServer(new PacketSetKeystoneCombo(comboIndex));
            return true;
        }
        ...
        while (matchIndex < combo.metas.length && searchIndex < InventoryKeyStone.inventorySize + (!runeBag.isEmpty() ? InventoryRuneBag.inventorySize : 0)) {
            ...
            if (stack != ItemStack.EMPTY && stack.getItemDamage() == combo.metas[matchIndex]) {
                swapInventorySlots(keyStoneInventory, searchInventory, matchIndex, inventoryIndex);
                matchIndex++;
                searchIndex = matchIndex;
                continue;
            } else if (combo.metas[matchIndex] == -1) {
                swapInventorySlots(keyStoneInventory, searchInventory, matchIndex, inventoryIndex);
                matchIndex++;
                searchIndex = matchIndex;
                continue;
