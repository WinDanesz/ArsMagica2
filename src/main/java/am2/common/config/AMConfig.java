...
public class AMConfig extends Configuration {
    ...
    private final String KEY_BossPotionImmunities = "boss_potion_immunities";
    ...
    private Set<String> bossPotionImmunities = Collections.emptySet();
    private Map<String, Set<String>> bossSpecificPotionImmunities = Collections.emptyMap();
    ...
    private void loadBossPotionImmunities() {
        this.bossPotionImmunities = new HashSet<>();
        this.bossSpecificPotionImmunities = new HashMap<>();
        for (String entry : this.get(CATEGORY_BOSSES, this.KEY_BossPotionImmunities,
                new String[]{"ebwizardry:mind_trick", "ebwizardry:mind_control"},
                "Potion immunities for AM2 guardian bosses. Use potion_id for all AM2 bosses, or potion_id=entity_id for only the named boss. Both IDs use modid:registry_name. Example: ebwizardry:mind_control=arsmagica2:nature_guardian. Empty list disables these additional immunities; built-in Silence immunity remains.").getStringList()) {
            if (entry.trim().isEmpty()) continue;
            String[] ids = entry.split("=", -1);
            String potionId = ids[0].trim();
            String entityId = ids.length == 2 ? ids[1].trim() : "";
            String pattern = "[a-z0-9_.-]+:[a-z0-9/._-]+";
            if (ids.length > 2 || !potionId.matches(pattern) || (ids.length == 2 && !entityId.matches(pattern))) {
                ArsMagica.LOGGER.warn("Invalid boss potion immunity '{}'. Expected potion_id or potion_id=entity_id. Skipping.", entry);
                continue;
            }
            if (ids.length == 1) this.bossPotionImmunities.add(potionId);
            else this.bossSpecificPotionImmunities.computeIfAbsent(entityId, key -> new HashSet<>()).add(potionId);
        }
    }
    ...
    public boolean isBossPotionImmune(ResourceLocation potionId, ResourceLocation entityId) {
        if (potionId == null) return false;
        if (this.bossPotionImmunities.contains(potionId.toString())) return true;
        Set<String> immunities = entityId == null ? null : this.bossSpecificPotionImmunities.get(entityId.toString());
        return immunities != null && immunities.contains(potionId.toString());
    }
