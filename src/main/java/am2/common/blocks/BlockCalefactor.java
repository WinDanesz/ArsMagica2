...
public class BlockCalefactor extends BlockAMPowered {
    ...
    public BlockCalefactor() {
        ...
        // Enable JSON model rendering (TESR will render on top)
    ...
    public void onEntityWalk(World worldIn, BlockPos pos, Entity entityIn) {
        ...
        if (te instanceof TileEntityCalefactor) {
            TileEntityCalefactor calefactor = (TileEntityCalefactor) te;
            // Check if the calefactor is actively cooking
            if (calefactor.isCooking() && entityIn instanceof EntityLivingBase && !entityIn.isImmuneToFire()) {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (!worldIn.isRemote) {
            ...
            if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable<?>) worldIn.getTileEntity(pos), playerIn, KeystoneAccessType.USE)) {
                ...
                playerIn.openGui(ArsMagica.instance, IDDefs.GUI_CALEFACTOR, worldIn, pos.getX(), pos.getY(), pos.getZ());
            }
        }
        return true;
    }
