...
public class BlockSpellSealedDoor extends BlockDoor implements ITileEntityProvider {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
        ...
        if (worldIn.getBlockState(pos.down()).getBlock() == AMBlocks.spell_sealed_door)
            pos = pos.down();
        ...
        if (!worldIn.isRemote) {
            ...
            if (KeystoneUtilities.instance.canPlayerAccess((IKeystoneLockable<?>) te, playerIn, KeystoneAccessType.USE)) {
                if (playerIn.isSneaking()) {
                    playerIn.openGui(ArsMagica.instance, IDDefs.GUI_SPELL_SEALED_DOOR, worldIn, pos.getX(), pos.getY(), pos.getZ());
                } else {
                    return false;
                }
            }
...
//	}
    ...
    public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player,
        ...
        if (world.getBlockState(pos.down()).getBlock() == AMBlocks.spell_sealed_door)
            pos = pos.down();
    ...
    public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
        return new ItemStack(AMItems.spell_sealed_door);
    }

    @Override
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return AMItems.spell_sealed_door;
    }
    ...
    public boolean applyComponentToDoor(IBlockAccess access, SpellComponent component, BlockPos pos) {
        ...
        if (access.getBlockState(pos.down()).getBlock() == AMBlocks.spell_sealed_door)
            pos = pos.down();
