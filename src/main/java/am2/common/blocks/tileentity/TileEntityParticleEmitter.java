...
import am2.network.packets.PacketParticleEmitterUpdate;
...
public class TileEntityParticleEmitter extends TileEntity implements ITickable {
    ...
    public void update() {
        ...
        if (!show && !world.isRemote && ((forceShow && showTicks++ > 100) || !forceShow)) {
            showTicks = 0;
            forceShow = false;
            for (EntityPlayer player : world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(pos).expand(8D, 0, 0))) {
                if (player != null && !player.inventory.getCurrentItem().isEmpty() && player.inventory.getCurrentItem().getItem() == AMItems.crystal_wrench) {
    ...
    private void doSpawn() {
        ...
        AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, AMParticle.particleTypes[particleType], x, y, z);
        if (particle != null) {
            particle.AddParticleController(ArsMagica.proxy.particleManager.createDefaultParticleController(particleBehaviour, particle, new AMVector3(x, y, z).toVec3D(), speed, getBlockMetadata()));
    ...
    public void setShow(boolean show) {
        ...
        if (world.isRemote && show) {
            forceShow = false;
            showTicks = 0;
            EntityPlayer localPlayer = ArsMagica.proxy.getLocalPlayer();
            if (localPlayer != null && !localPlayer.inventory.getCurrentItem().isEmpty() && localPlayer.inventory.getCurrentItem().getItem() == AMItems.crystal_wrench) {
                AMVector3 myLoc = new AMVector3(pos);
...
//		int oldMeta = getBlockMetadata();
    ...
    public void syncWithServer() {
        if (this.world.isRemote) {
            NBTTagCompound compound = new NBTTagCompound();
            this.writeToNBT(compound);
            AMNetworkHandler.getNetwork().sendToServer(new PacketParticleEmitterUpdate(this.pos, compound));
        }
    }
}
