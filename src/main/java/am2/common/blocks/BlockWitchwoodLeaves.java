...
public class BlockWitchwoodLeaves extends BlockLeaves {
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return Item.getItemFromBlock(AMBlocks.witchwood_sapling);
    }
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (!ArsMagica.config.witchwoodLeafPFX())
            return;
        if (rand.nextInt(300) == 0 && worldIn.isAirBlock(pos.down())) {
            AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(worldIn, "witchwood_leaf", pos.getX() + rand.nextDouble(), pos.getY() + rand.nextDouble(), pos.getZ() + rand.nextDouble());
            if (particle != null) {
