...
public class Wave extends SpellShape {
    ...
    public SpellCastResult beginStackStage(SpellData spell, EntityLivingBase caster, EntityLivingBase target, World world, double x, double y, double z, EnumFacing side, boolean giveXP, int useCount) {
        ...
        double radius = spell.getModifiedValue(ArsMagica.config.getWaveDefaultRadius(), SpellModifiers.RADIUS, Operation.ADD, world, caster, target);
        double speed = spell.getModifiedValue(1, SpellModifiers.SPEED, Operation.ADD, world, caster, target) * ArsMagica.config.getWaveSpeedMultiplier();
        int duration = (int) spell.getModifiedValue(ArsMagica.config.getWaveDefaultDuration(), SpellModifiers.DURATION, Operation.MULTIPLY, world, caster, target);
        ...
        wave.setPosition(x, y + 0.5, z);
        wave.setWave(caster.rotationYaw, (float) speed);
        wave.noClip = hasPiercing;
        wave.setGravity((float) (gravityModifiers * ArsMagica.config.getWaveGravityPerModifier()));
