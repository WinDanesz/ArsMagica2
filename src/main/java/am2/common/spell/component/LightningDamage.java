...
public class LightningDamage extends SpellComponent {
    ...
    public boolean applyEffectEntity(SpellData spell, World world, EntityLivingBase caster, Entity target) {
        ...
        double damage = spell.getModifiedValue(ArsMagica.config.getLightningDamageBase(), SpellModifiers.DAMAGE, Operation.ADD, world, caster, target);
    ...
    public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier) {
        for (int i = 0; i < 5; ++i) {
            AMParticle particle = (AMParticle) ArsMagica.proxy.particleManager.spawn(world, "lightning_hand", x, y, z);
            if (particle != null) {
