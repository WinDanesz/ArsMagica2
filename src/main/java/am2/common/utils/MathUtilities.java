...
public class MathUtilities {
    ...
    public static boolean isInFieldOfVision(Entity e1, EntityLivingBase e2, float angleX, float angleY) {
        // Calculate the yaw and pitch from e2 to e1
        double dx = e1.posX - e2.posX;
        double dy = (e1.posY + e1.height / 2.0) - (e2.posY + e2.getEyeHeight());
        double dz = e1.posZ - e2.posZ;
        double distXZ = Math.sqrt(dx * dx + dz * dz);

        // Yaw: atan2 gives angle from +Z axis, Minecraft yaw is degrees from +Z clockwise
        float targetYaw = (float) (Math.toDegrees(Math.atan2(-dx, dz)));
        float targetPitch = (float) (Math.toDegrees(-Math.atan2(dy, distXZ)));

        // Normalize the angular differences to [-180, 180]
        float yawDiff = MathHelper.wrapDegrees(targetYaw - e2.rotationYaw);
        float pitchDiff = MathHelper.wrapDegrees(targetPitch - e2.rotationPitch);

        return Math.abs(yawDiff) <= angleX && Math.abs(pitchDiff) <= angleY;
    }
