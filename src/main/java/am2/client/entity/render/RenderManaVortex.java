...
public class RenderManaVortex extends Render<EntityManaVortex> {
    ...
    private static final ResourceLocation wisp = new ResourceLocation("arsmagica2", "textures/entities/wisp.png");
    ...
    private void renderArsMagicaEffect(Tessellator tessellator, int ticks, int life, float rotation, float scale, float percent) {
        ...
        GL11.glColor4f(0.3f, 0.4f, 1.0f, 0.7f);

        Minecraft.getMinecraft().renderEngine.bindTexture(wisp);
        GL11.glPushMatrix();
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        GL11.glTranslatef(0.0f, 0.25f, 0.0f);
        GL11.glRotatef(-rotation * 0.5f, 0.0f, 0.0f, 1.0f);
        GL11.glScalef(scale * 2, scale * 2, scale * 2);
        GL11.glTranslatef(0.0f, -0.25f, 0.0f);
        renderSprite(tessellator);
        GL11.glPopMatrix();
        ...
        for (int var7 = 0; var7 < (var4 + var4 * var4) / 2.0F * 60.0F; ++var7) {
            ...
            tessellator.getBuffer().pos(0.0D, 0.0D, 0.0D).color((int) (percent * 255), (int) (255 - (percent * 255)), (int) (255 - (percent * 255)), (int) (255.0F * (0.2F - (var5 * 0.2f)))).endVertex();
