...
import am2.network.packets.PacketSaveKeystoneCombo;
...
public class GuiKeystone extends GuiContainer {
    ...
    private static final ResourceLocation background = new ResourceLocation(ArsMagica.MODID, "textures/gui/keystone_gui.png");
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        hoveredCombo = -1;
        int numCombos = ItemKeystone.numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack());
    ...
    private void recalculateSlider() {
        int sliderMax = Math.max(0, ItemKeystone.numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack()) - 9);
        ...
        if (button == addCombination) {
            if (combinationName.getText() == null || combinationName.getText().trim().equals("")) {
                ...
                for (int i = 0; i < InventoryKeyStone.inventorySize; ++i) {
                    ...
                    metas[i] = !stack.isEmpty() ? stack.getItemDamage() : -1;
                }
                AMNetworkHandler.getNetwork().sendToServer(new PacketSaveKeystoneCombo(true, combinationName.getText(), metas));

                ItemKeystone.addCombination(((ContainerKeystone) this.inventorySlots).getKeystoneStack(), combinationName.getText(), metas);
            ...
            if (matchedCombo == null) {
                ...
                AMNetworkHandler.getNetwork().sendToServer(new PacketSaveKeystoneCombo(false, matchedCombo.name, matchedCombo.metas));

                ItemKeystone.removeCombination(((ContainerKeystone) this.inventorySlots).getKeystoneStack(), matchedCombo.name);

                recalculateSlider();
            }
        } else {

            int numCombinatons = ItemKeystone.numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack());
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        int numCombos = Math.min(ItemKeystone.numCombinations(((ContainerKeystone) this.inventorySlots).getKeystoneStack()), comboScrollOffset + 9);
        ...
        for (int i = comboScrollOffset; i < numCombos; ++i) {
            KeystoneCombination combo = ((ItemKeystone) AMItems.keystone).getCombinationAt(((ContainerKeystone) this.inventorySlots).getKeystoneStack(), i);
            ...
            for (int n = 0; n < combo.metas.length; ++n) {
                if (combo.metas[n] > -1) {
                    TextureAtlasSprite icon = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getParticleIcon(AMItems.rune, combo.metas[n]);
                    AMGuiHelper.DrawIconAtXY(icon, cx, cy, this.zLevel, 16, 16, true);
                }
                cx += 18;
            }
        ...
        if (hoveredCombo > -1) {
            KeystoneCombination combo = ((ItemKeystone) AMItems.keystone).getCombinationAt(((ContainerKeystone) this.inventorySlots).getKeystoneStack(), hoveredCombo);
