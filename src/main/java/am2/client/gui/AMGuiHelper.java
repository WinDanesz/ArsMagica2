...
public class AMGuiHelper {
    ...
    // Compendium link tracking
    private static final List<CompendiumLinkInfo> compendiumLinks = new ArrayList<>();
    // Last opened compendium entry state (for reopen-to-same-page)
    private static CompendiumEntry lastCompendiumEntry = null;
    private static int lastCompendiumEntryPage = 0;
    //=========================================
    ...
    public static class CompendiumLinkInfo {
        public final int x, y, width, height;
        public final String linkText;

        public CompendiumLinkInfo(int x, int y, int width, int height, String linkText) {
            this.x = x;
            this.y = y;
            this.width = width;
            this.height = height;
    ...
    public static void clearCompendiumLinks() {
        compendiumLinks.clear();
    }

    public static List<CompendiumLinkInfo> getCompendiumLinks() {
        return new ArrayList<>(compendiumLinks);
    }
    ...
    public static void saveCompendiumEntryState(CompendiumEntry entry, int page) {
        lastCompendiumEntry = entry;
        lastCompendiumEntryPage = page;
    }

    public static void clearCompendiumEntryState() {
        lastCompendiumEntry = null;
        lastCompendiumEntryPage = 0;
    }

    public static void OpenCompendiumGui(ItemStack stack) {
        if (lastCompendiumEntry != null) {
            GuiArcaneCompendium gui = new GuiArcaneCompendium(lastCompendiumEntry, lastCompendiumEntryPage);
    ...
    public static void DrawIconAtXY(TextureAtlasSprite IIcon, float x, float y, float zLevel, int w, int h, boolean semitransparent) {
        ...
        // Fix GUI texture warping: Save all GL state before modifying texture matrix
        GlStateManager.pushMatrix();
        GlStateManager.matrixMode(GL11.GL_TEXTURE);
        GlStateManager.pushMatrix();
        GlStateManager.loadIdentity(); // Fix GUI texture warping: Reset texture matrix
        ...
        // Fix GUI texture warping: Restore texture matrix properly
        GlStateManager.matrixMode(GL11.GL_TEXTURE);
    ...
    public static void DrawIconAtXY(TextureAtlasSprite IIcon, float x, float y, float zLevel, int w, int h, int color) {
        ...
        // Fix GUI texture warping: Save GL state and reset texture matrix
        GlStateManager.pushMatrix();
        GlStateManager.matrixMode(GL11.GL_TEXTURE);
        GlStateManager.pushMatrix();
    ...
    public static void DrawItemAtXY(ItemStack stack, float x, float y, float zLevel, float scale) {
        ...
        if (scale != 1.0f) {
            ...
            } else if (stack.getItem().equals(AMItems.etherium)) {
                Minecraft.getMinecraft().renderEngine.bindTexture(LOCATION_BLOCKS_TEXTURE);
    ...
    // State for tracking links across words
    private static StringBuilder currentLinkText = null;
    private static int currentLinkStartX = 0;
    private static int currentLinkStartY = 0;
    ...
    public static void drawCompendiumText(String text, int x_start, int y_start, int max_width, int start_color, FontRenderer fontRenderer) {
        // Don't clear links here - they should be cleared at the start of the entire page render
        ...
        for (String word : words) {
            ...
            // Strip newlines before calculating word width
            ...
            // Skip empty words (from standalone \n tokens)
            if (word.isEmpty()) {
                if (linesAfter > 0) {
            ...
            cur_color = parseColorAndDrawWithLinks(word, posX, posY, cur_color, fontRenderer);
        ...
        // Finalize any open link at the end
        if (currentLinkText != null && currentLinkText.length() > 0) {
    ...
    private static int parseColorAndDrawWithLinks(String word, int posX, int posY, int cur_color, FontRenderer fontRenderer) {
        ...
        while (index > -1 && index < word.length() - 1) {
            ...
            // Add to link text if we're currently tracking a link
            if (currentLinkText != null && !toRender.isEmpty()) {
                if (currentLinkText.length() > 0) {
                    currentLinkText.append(" ");
                }
                currentLinkText.append(toRender);
            }
            ...
            // Track start of link (#5 = magenta)
            if (nextChar == '5' && currentLinkText == null) {
                currentLinkText = new StringBuilder();
                currentLinkStartX = posX;
                currentLinkStartY = posY;
            }
            // Track end of link (#0 = black)
            else if (nextChar == '0' && currentLinkText != null) {
                int linkWidth = posX - currentLinkStartX;
                String linkTextStr = currentLinkText.toString().trim();
                if (linkWidth > 0 && !linkTextStr.isEmpty()) {
                    compendiumLinks.add(new CompendiumLinkInfo(currentLinkStartX, currentLinkStartY, linkWidth, fontRenderer.FONT_HEIGHT, linkTextStr));
                }
                currentLinkText = null;
            }
        ...
        // Capture remaining text if we're in a link
        if (currentLinkText != null && !word.isEmpty()) {
            if (currentLinkText.length() > 0) {
                currentLinkText.append(" ");
            }
            currentLinkText.append(word);
        }
...
//		}
    ...
    public static void overrideKeyboardInput() {
        ...
        if (mc.player != null && mc.world != null && EntityExtension.For(mc.player).shouldReverseInput()) {
            ...
            if (mc.player.isPotionActive(AMPotions.scramble_synapses)) {
