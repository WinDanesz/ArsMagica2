...
import am2.network.packets.PacketParticleSpawnSpecial;
...
public class ParticleManagerServer {
    ...
    public Object spawn(World world, String name, double x, double y, double z, double targetX, double targetY, double targetZ) {
        if (!world.isRemote) {
            ...
            writer.add(targetY);
            writer.add(targetZ);
            AMNetworkHandler.getNetwork().sendToAllAround(new PacketParticleSpawnSpecial(writer.generate()), new NetworkRegistry.TargetPoint(world.provider.getDimension(), x, y, z, 32));
        }
        return null;
    }
    ...
    public Object spawn(World world, String name, Entity source, Entity target) {
        if (!world.isRemote) {
            ...
            writer.add(target.getEntityId());
            AMNetworkHandler.getNetwork().sendToAllAround(new PacketParticleSpawnSpecial(writer.generate()), new NetworkRegistry.TargetPoint(world.provider.getDimension(), source.posX, source.posY, source.posZ, 32));
        }
        return null;
    }
    ...
    public void BoltFromEntityToEntity(World world, Entity caster, Entity source, Entity target, int damage, int type, int color) {
        if (!world.isRemote) {
            ...
            writer.add(type);
            writer.add(color);
            AMNetworkHandler.getNetwork().sendToAllAround(new PacketParticleSpawnSpecial(writer.generate()), new NetworkRegistry.TargetPoint(world.provider.getDimension(), caster.posX, caster.posY, caster.posZ, 32));
        }
    }
    ...
    public void BoltFromPointToPoint(World world, double startX, double startY, double startZ, double endX, double endY, double endZ, int type, int color) {
        if (!world.isRemote) {
            ...
            writer.add(type);
            writer.add(color);
            AMNetworkHandler.getNetwork().sendToAllAround(new PacketParticleSpawnSpecial(writer.generate()), new NetworkRegistry.TargetPoint(world.provider.getDimension(), startX, startY, startZ, 32));
        }
    }
    ...
    public Object BeamFromEntityToPoint(World world, Entity source, double endX, double endY, double endZ, int color) {
        if (!(source instanceof EntityPlayer) || source == ArsMagica.proxy.getLocalPlayer()) {
