...
public class AMParticleIcons {
    ...
    public void init(TextureStitchEvent.Pre e) {
        ...
        loadAndInitIcon("sorcery_hand", "sorcery_hand", textureMap);
        loadAndInitIcon("healing_hand", "healing_hand", textureMap);
    ...
    private void loadAndInitIcon(String name, String TextureAtlasSpritePath, TextureMap register, boolean registerName) {
        ResourceLocation location = new ResourceLocation("arsmagica2", "items/particles/" + TextureAtlasSpritePath);
        TextureAtlasSprite icon = register.registerSprite(location);
        if (registerName) {
            if (icon != null) {
                icons.put(name, icon);
            }
    ...
    public TextureAtlasSprite getIconByName(String name) {
        ...
        // Check if icon is null or is the missing sprite
        TextureAtlasSprite missingSprite = Minecraft.getMinecraft().getTextureMapBlocks().getMissingSprite();
        if (icon == null || icon == missingSprite) {
            // Fall back to lights texture
            icon = icons.get("lights");
            if (icon == null || icon == missingSprite) {
