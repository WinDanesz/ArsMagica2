...
public class RibbonCurve {
    ...
    void addSegment() {
        ...
        TextureAtlasSprite icon = AMParticleIcons.instance.getIconByName("symbols");
        // Fallback to "lights" if symbols icon is null or missing
        if (icon == null) {
            icon = AMParticleIcons.instance.getIconByName("lights");
        }

        Quad3D q = new Quad3D(p0, p1, p2, p3, icon);
        quads.add(q);
    }
