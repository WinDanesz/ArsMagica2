...
public class ItemRenderer {
	...
	public void renderItemEvent(RenderingItemEvent event) {
		...
		TRSRTransformation transform =  null; // todo  ModelUtils.DEFAULT_BLOCK_STATE.apply(Optional.fromNullable(event.getCameraTransformType())).orNull();
		if (transform != null) {
...
//						|| event.getCameraTransformType() == TransformType.THIRD_PERSON_LEFT_HAND);
		tesr.render(te, 0, 0, 0, event.getStack().getItemDamage(), -10, 1f);
