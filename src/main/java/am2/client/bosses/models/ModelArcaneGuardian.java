...
//			Sigil.render(f5);
    ...
    private void updateRotations(EntityArcaneGuardian guardian, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        // Use a consistent rotation speed - the entity handles smooth interpolation already
        ...
        // Arm animation using synced cast ticks from DataManager
        float armRotation = 0;
        float pants_rotation = 0;

        int castTicks = guardian.getCastTicks();
        float partialTicks = f2 - guardian.ticksExisted;

        if (castTicks > 0) {
            // Casting animation - arm raises up then comes forward
            ...
            float smoothTicks = castTicks + partialTicks;

            if (smoothTicks < action_ticks) {
                // Phase 1: Raise arm back
                armRotation = (float) Math.toRadians(-max_degrees_x * (smoothTicks / action_ticks));
            } else if (smoothTicks < action_ticks + fast_action_ticks) {
                // Phase 2: Thrust forward
                armRotation = (float) Math.toRadians(-max_degrees_x + (final_degrees_x * ((smoothTicks - action_ticks) / fast_action_ticks)));
                pants_rotation = (float) Math.toRadians(-max_pants_degrees * ((smoothTicks - action_ticks) / fast_action_ticks));
            } else {
                // Phase 3: Hold position
                armRotation = (float) Math.toRadians(-max_degrees_x + final_degrees_x);
                if (smoothTicks < action_ticks + fast_action_ticks + final_action_ticks) {
                    pants_rotation = (float) Math.toRadians(-max_pants_degrees + (max_pants_degrees * ((smoothTicks - action_ticks - fast_action_ticks) / final_action_ticks)));
                }
            }
        } else if (castTicks < 0) {
            // Return animation - smoothly lower arm based on remaining negative ticks
            float returnProgress = Math.min(1.0f, (-castTicks + partialTicks) / 20.0f);
            float holdPosition = (float) Math.toRadians(-80); // -160 + 80
            armRotation = holdPosition * (1.0f - returnProgress);
        }

        RightUpperArm.rotateAngleX = RightUpperArm.getRestRotationX() + armRotation;
        RightForearm.rotateAngleX = RightForearm.getRestRotationX() + armRotation;
        RightHand.rotateAngleX = RightHand.getRestRotationX() + armRotation;
        Wand.rotateAngleX = Wand.getRestRotationX() + armRotation;
