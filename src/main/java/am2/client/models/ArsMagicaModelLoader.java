...
public class ArsMagicaModelLoader implements ICustomModelLoader {
    ...
    public boolean accepts(ResourceLocation modelLocation) {
        // Only accept spell_book, not individual spell icons (they use standard models)
        return modelLocation.toString().contains("/spell_bookf");
    }
...
//        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/obelisk_active_highpower"));
        ...
        //e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/calefactor"));
        //e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/essence_conduit"));
        //e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/keystone_receptacle"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/keystone_chest"));
//        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/blockseerstone"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/magicians_workbench"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/summoner"));
       // e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/lectern"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/astral_barrier"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/custom/otheworld_aura"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/everstone"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/rune_stone"));
        e.getMap().registerSprite(new ResourceLocation("arsmagica2:blocks/caster_rune_side"));

        // Register spell icons for texture atlas
        for (ResourceLocation spellIcon : spellIcons) {
            e.getMap().registerSprite(spellIcon);
        }
    }
    ...
    public static List<ResourceLocation> getResourceListing() {
        ArrayList<ResourceLocation> toReturn;
        try {
            URI uri = ArsMagica.class.getResource(iconsPath).toURI();
            Path myPath;
    ...
    private static ArrayList<ResourceLocation> processDirectory(Path dir, FileSystem fs) {
        ...
        try (Stream<Path> walk = Files.walk(dir, 1)) {
            for (Iterator<Path> file = walk.iterator(); file.hasNext(); ) {
                ...
                // Only include .png files, strip extension completely
                if (name.endsWith(".png")) {
                    // Remove .png extension
                    String nameWithoutExt = name.substring(0, name.length() - 4);
                    // Skip files that aren't valid texture names (no extra dots/spaces suggest temp/backup files)
                    if (!nameWithoutExt.contains(" ") || nameWithoutExt.matches("^[\\d]+$|^[a-z0-9-]+$")) {
                        toReturn.add(new ResourceLocation("arsmagica2:" + iconsPrefix + nameWithoutExt));
                    }
                }
            }
        } catch (IOException e) {
