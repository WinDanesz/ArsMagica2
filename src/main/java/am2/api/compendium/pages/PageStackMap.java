            ...
            for (Entry<ItemStack, Integer> entry : map.entrySet()) {
                if (entry.getValue() != null && entry.getValue().intValue() == i) {
                    ...
                    if (entryPage == page) {
                        ...
                        mc.fontRenderer.drawStringWithShadow(entry.getValue().toString(), posX + offsetX + 8 - (mc.fontRenderer.getStringWidth(entry.getValue().toString()) / 2), posY + offsetY + 18, 0xffffff);
                    }
