...
public class PageRitual extends CompendiumPage<IRitualInteraction.Wrapper> {
    ...
    protected void renderPage(int posX, int posY, int mouseX, int mouseY) {
        String ritualName;
        if (element.getRitualInteraction() instanceof SpellPart)
            ritualName = TextFormatting.UNDERLINE.toString() + TextFormatting.BOLD.toString() + I18n.format("ritual." + ((SpellPart) element.getRitualInteraction()).getRegistryName().toString() + ".name");
        else
...
//		mc.fontRenderer.drawString(shapeName, posX + 72 - (mc.fontRenderer.getStringWidth(shapeName) / 2), posY + 30, 0);
        ...
        int yOffset = 0;
        ItemStack stackTip = ItemStack.EMPTY;
