...
public class TypedMultiblockGroup extends MultiblockGroup {
    ...
    private BiPredicate<World, BlockPos> fallbackMatcher;
    ...
    /**
     * Sets a fallback matcher that is tried when no explicit variant matches.
     * The predicate receives (world, startCheckPos) and should return true if the
     * positions satisfy an alternative matching criterion (e.g. ore dictionary).
     */
    public void setFallbackMatcher(BiPredicate<World, BlockPos> fallbackMatcher) {
    ...
    public boolean matches(World world, BlockPos startCheckPos) {
        ...
        if (fallbackMatcher != null) {
            return fallbackMatcher.test(world, startCheckPos);
        }
        return false;
    }
}
