...
public class RitualShapeHelper {
    ...
    public boolean matchesRitual(IRitualInteraction ritual, World world, BlockPos pos) {
        ...
        if (!ritual.getRitualShape().matches(world, pos)) {
            LogHelper.info("Ritual shape does not match at position %s", pos);
            return false;
        }
        LogHelper.info("Ritual shape matched! Checking reagents...");
        for (ItemStack stack : ritual.getRitualReagents()) {
            ...
            if (!matches) {
                LogHelper.info("Missing reagent: %s", stack.toString());
                return false;
            }
        }
        LogHelper.info("All ritual requirements met!");
        return true;
    }
    ...
    private void corruptionRitual() {
        ...
        corruptionMap.put(0, AMBlocks.wizard_chalk.getDefaultState());
        corruptionMap.put(1, AMBlocks.warding_candle.getDefaultState());
    ...
    private void purificationRitual() {
        MultiblockGroup chalks = new MultiblockGroup("chalk", Lists.newArrayList(AMBlocks.wizard_chalk.getDefaultState()), true);
        MultiblockGroup candles = new MultiblockGroup("candle", Lists.newArrayList(AMBlocks.warding_candle.getDefaultState()), true);
    ...
    private void hourglassRitual() {
        MultiblockGroup chalks = new MultiblockGroup("chalk", Lists.newArrayList(AMBlocks.wizard_chalk.getDefaultState()), true);
        MultiblockGroup chalks_rotated = new MultiblockGroup("chalk_rot", Lists.newArrayList(AMBlocks.wizard_chalk.getDefaultState()), true);
